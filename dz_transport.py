...
while True:
    ...
    if senders_city == "b" and recipient_city == "b":
        ...
        ts.earnings(summ)
        reg[i] = {f"город отправитель:{senders_city}, город получатель:{recipient_city}, Путь:{distance}км, вес груза:{load_size} кг, заработано:{summ}$"}
        ...
        ts = Train(distance)
        ts.earnings(summ)
        reg[i] = {f"Вид т/c: поезд, город отправитель:{senders_city}, город получатель:{recipient_city}, Путь:{distance}км, вес груза:{load_size} кг, заработано:{summ}$"}
        ...
        ts = Car(distance)
        ts.earnings(summ)
        reg[i] = {f"Вид т/c: авто, город отправитель:{senders_city}, город получатель:{recipient_city}, Путь:{distance}км, вес груза:{load_size} кг, заработано:{summ}$"}
    else:
        print(*reg)
        break
    i += 1
